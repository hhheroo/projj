...
class Command extends BaseCommand {
  ...
  async run({ cwd, rawArgv, argv }) {
    try {
      await this.init();
      await this._run(cwd, rawArgv, argv);
  ...
  async addRepo(repo, cacheKey, argv) {
    // preadd hook
    await this.runHook('preadd', cacheKey);

    const targetPath = cacheKey;
    const command = `git clone ${repo} ${targetPath} ${argv.join(' ')}`
    ...
    await this.runScript(`${command} > /dev/null`, {
      env,
    });
